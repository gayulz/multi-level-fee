...
public class DataInitializer implements ApplicationRunner {
	...
	public void run(ApplicationArguments args) {
		// ====================================================
		// 최초 실행 여부 판단: DB에 User가 1명 이상 있으면 SKIP
		// ====================================================
		if (userRepository.count() > 0) {
			log.info("========================================");
			log.info("[DataInitializer] 기존 유저 데이터가 존재합니다. 초기화를 건너뜁니다.");
			log.info("========================================");
			return;
		}

		log.info("========================================");
		log.info("[DataInitializer] DB 정리 시작 (유저가 없어 기존 쓰레기 조직/정산 등을 포맷합다)");
		settlementRequestRepository.deleteAllInBatch();
		settlementNodeRepository.deleteAllInBatch();
		userRepository.deleteAllInBatch();
		organizationRepository.deleteAllInBatch();
		log.info("[DataInitializer] 최초 실행 - 초기 더미 데이터 주입 시작!");
		log.info("========================================");
